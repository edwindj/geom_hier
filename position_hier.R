...
position_hier <- function(width = NULL, preserve = c("total", "single")) {
  ggproto(NULL, PositionHier,
          width = width,
...
#' @usage NULL
#' @export
PositionHier <- ggproto("PositionHier", Position,
                         width = NULL,
                         preserve = "total",
...
pos_hier <- function(df, width, n = NULL) {
  #browser()
  if (is.null(n)) {
  ...
  # TODO add option to control the minimum width
  widths <- d_width * (groupidx/n)
  df$xmin <- df$x - widths/2
  df$xmax <- df$x + widths/2
  print(list(pos_hier=df))
  df
}